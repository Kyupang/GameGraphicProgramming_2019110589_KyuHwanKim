...
namespace library
{
      ...
      void CleanupDevice()
    {
        ...
        if (g_pRenderTargetView.Get()) g_pRenderTargetView.Get();
        if (g_pSwapChain1.Get()) g_pSwapChain1.Get();
        if (g_pSwapChain.Get()) g_pSwapChain.Get();
        if (g_pImmediateContext1.Get()) g_pImmediateContext1.Get();
        if (g_pImmediateContext.Get()) g_pImmediateContext.Get();
        if (g_pd3dDevice1.Get()) g_pd3dDevice1.Get();
        if (g_pd3dDevice.Get()) g_pd3dDevice.Get();
    }
